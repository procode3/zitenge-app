...
const CartItem = ({ item }) => {
  ...
  const { rack, shelfColor, frameColor, price } = item;
    ...
    <div className='flex justify-between gap-2 items-center w-full py-4  px-4 '>
      <div className='flex gap-2 md:gap-8 items-center w-2/3'>
        ...
        <div className='flex flex-col gap-0 w-1/2'>
          <h2 className='text-sm font-semibold'>{`${rack.toLowerCase()} shoe rack`}</h2>
          <p className='text-xs opacity-50'>
            {frameColor === shelfColor
              ? frameColor
              : `${frameColor} on ${shelfColor}`}{' '}
          </p>
